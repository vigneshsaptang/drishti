  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5 px-1">
      {items.map(row => (
        ...
        >
          <p className="text-xs font-mono uppercase tracking-[0.15em] text-sap-muted font-medium">{row.label}</p>
          <p className="text-2xl font-mono font-bold text-sap-text tabular-nums mt-0.5">{row.value}</p>
          <p className="text-xs font-mono text-sap-dim">{row.sub}</p>
        </div>
      ))}
    </div>
  );
